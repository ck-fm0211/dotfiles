...
ただし、グローバル設定では `Bash(make:*)` を明示的に許可している。これは `deny` ルールより先に評価されるため、make 経由で呼ばれるスクリプトは内部で `rm`・`sudo`・`curl` などを使っていても deny を実質バイパスできる。この挙動は意図的な設計であり、`scripts/` 配下のセットアップスクリプトを正常動作させるために必要である。
